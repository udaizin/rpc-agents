...
OUTPUT_DIR = f'./JudgeLLM/result/eval/anthropic_direct_only_target'

normal_setting_prompt = '''
あなたは心理学者として、{target_interlocutor_id}の発言を分析し、Big Five性格特性を予測する任務を担当しています。
'''
...
それでは、発言の分析と性格特性の評価を始めてください。
'''

def load_dialogue_list(path, target_interlocutor_id):
    ...
    for dialogue in dialogues_json:
        ...
        dialogue_text = '\n'.join([f"{utterance['role']}: {utterance['content']}" for utterance in utterances if utterance['role'] == target_interlocutor_id])
...
if __name__ == '__main__':
    ...
    for target_interlocutor_id in tqdm(TARGET_INTERLOCUTOR_IDS):
        ...
        dialogue_list = load_dialogue_list(f'./JudgeLLM/data/sample20/{target_interlocutor_id}_extracted_data.json', target_interlocutor_id)
