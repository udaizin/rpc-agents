...
OUTPUT_DIR = f'./JudgeLLM/result/tmp'
...
def get_answer(dialogue_list, question):
    ...
    user_prompt = questionnaire_prompt.format(dialogues=dialogues, question=question)

    completion = client.chat.completions.create(
        # TODO: 実験のためにgpt-4o-miniに変更
        model="gpt-4o-mini",
        messages=[
...
def calculate_error(scores):
    ...
    for key in scores.keys():
        error[key] = abs(scores[key] - target_interlocutor_BFI[key])
