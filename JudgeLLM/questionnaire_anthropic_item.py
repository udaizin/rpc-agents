...
BIGFIVE_ITEMS = ['外向性', '神経症傾向', '開放性', '誠実性', '協調性']

# TODO: evalかvalidかはデータの種類によって違うので、適宜変更する
OUTPUT_DIR = f'./JudgeLLM/result/eval/anthropic_item_extracted'
...
あなたは対話履歴に基づいて、{target_interlocutor_id}のビッグファイブ性格特性の{bigfive_item}推測する役割を担っています。
...
この対話履歴を基に、上記の{bigfive_item}について、1から7までの数値（小数を含む）で評価してください。1は最も低い傾向を、7は最も高い傾向を示します。
...
def get_answer(dialogue_list, target_interlocutor_id, bigfive_item):
    dialogues = '\n\n'.join(dialogue_list)
    # プロンプトを作成
    ...
    user_prompt = questionnaire_prompt.format(target_interlocutor_id=target_interlocutor_id, dialogues=dialogues, bigfive_item=bigfive_item)
...
if __name__ == '__main__':
    ...
    for target_interlocutor_id in tqdm(TARGET_INTERLOCUTOR_IDS):
        ...
        for bigfive_item in BIGFIVE_ITEMS:
            # 対話データをAnthropicに投げて回答を取得
            answer = get_answer(dialogue_list, target_interlocutor_id, bigfive_item)
            # answerを解析してscoresに代入
            answer = answer.split('<result>')[1].split('</result>')[0].split('\n')
            answer = answer[1:-1]
            key, value = answer[0].split(': ')
