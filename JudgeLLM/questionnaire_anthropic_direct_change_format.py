...
OUTPUT_DIR = f'./JudgeLLM/result/eval/anthropic_direct_extracted_change_format'
...
def load_dialogue_list(path):
    ...
    for dialogue in dialogues_json:
        ...
        dialogue_text = '\n'.join([f"{utterance['role']}: {utterance['content']}" for utterance in utterances])
...
if __name__ == '__main__':
    ...
    for target_interlocutor_id in tqdm(TARGET_INTERLOCUTOR_IDS):
        ...
        dialogue_list = load_dialogue_list(f'./JudgeLLM/data/sample20/{target_interlocutor_id}_extracted_data.json')
