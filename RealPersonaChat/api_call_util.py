...
client = OpenAI(api_key=apikey_list[0])
...
@retry(retry=retry_if_exception_type(RateLimitError), wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
...
def decoder_for_local_chat(model_name, input, max_tokens, n=1, stop=None, **kwargs):
    ...
    unset_proxy()
    response = client.chat.completions.create(api_key="EMPTY",
        api_base=api_base,
        model=model_name,
        messages=[
            {"role": "user", "content": input},
...
def decoder_for_local_completion(model_name, input, max_tokens, n=1, stop=None, **kwargs):
    ...
    unset_proxy()
    response = client.completions.create(api_key="EMPTY",
        api_base=api_base,
        model=model_name,
        prompt=input,
        max_tokens=max_tokens,
